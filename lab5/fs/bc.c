...
bc_pgfault(struct UTrapframe *utf)
{
	...
	if(sys_page_alloc((envid_t)0, ROUNDDOWN(addr, PGSIZE), PTE_P | PTE_U | PTE_W) < 0 )
...
flush_block(void *addr)
{
	...
	void* addr_rd = ROUNDDOWN(addr, PGSIZE);
	if(!va_is_mapped(addr_rd) || !va_is_dirty(addr_rd))
		return;
	ide_write(blockno * BLKSECTS, addr_rd, BLKSECTS);
	sys_page_map(0, addr_rd, 0, addr_rd, PTE_SYSCALL);
	return;
}
