...
static struct Command commands[] = {
	...
	{ "c", "a GDB instruction, continue executution from the current location", mon_c},
	{ "si", "a GDB instruction, execute the code instruction by instruction", mon_si},
	{ "x", "a GDB instruction, display the memory ", mon_x},
};
...
int mon_si(int argc, char **argv, struct Trapframe *tf){
	if(tf){
		...
		debuginfo_eip((uintptr_t)tf->tf_eip, &debug_info);
		cprintf("tf_eip=%08x\n", tf->tf_eip);
		cprintf("%s:%u %.*s+%u\n", debug_info.eip_file, debug_info.eip_line, debug_info.eip_fn_namelen, debug_info.eip_fn_name, tf->tf_eip - (uint32_t)debug_info.eip_fn_addr);
		return -1;
	}
	cprintf("no user process\n");
	return 0;
...
int mon_x(int argc, char **argv, struct Trapframe *tf){
	if(tf){
		uintptr_t addr = (uintptr_t)strtol(argv[1], NULL, 16);
		uint32_t info;
		__asm __volatile("movl (%0), %0" : "=r" (info) : "r" (addr));
		cprintf("%d\n", info);
		return 0;
	}
	cprintf("no user process\n");
