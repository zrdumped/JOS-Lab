...
page_init(void)
{
	...
	// the pages used by pages(array) should be marked as allocated
	size_t allocated_num = PGNUM(PADDR(boot_alloc(0)));
	for(i; i < allocated_num; i++){
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
	//  4) Then extended memory [EXTPHYSMEM, ...).
	//left 16 pages for chunk
	for(i; i < npages - MAXCHUNKNUM * (1 << MAXORDER); i++){
		pages[i].pp_ref = 0;
		//add this page to free list
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
...
page_alloc(int alloc_flags)
{
	...
	struct Page* allocated_page = page_free_list;
	page_free_list = page_free_list->pp_link;
	allocated_page->pp_ref = 0;
...
chunk_init(void)
{
	...
	//all the chunks are initialized in the list with max order
	for(int i = npages - MAXCHUNKNUM * (1 << MAXORDER); i < npages; i+= (1 << MAXORDER)){
		for(int j = 0; j < (1 << MAXORDER); j++){
			pages[i + j].pp_ref = 0;
			pages[i + j].pp_link = NULL;
		}
		pages[i].pp_link = chunk_list[MAXORDER];
		chunk_list[MAXORDER] = &pages[i];
	}
...
chunk_alloc(int alloc_flags, int order)
{
	...
	// Fill this function in
	while(target_order <= MAXORDER && chunk_list[target_order] == NULL)
		target_order ++;
	//out of free memory
	//assert(chunk_list[target_order] == NULL);
	if(target_order > MAXORDER) return NULL;
	struct Page *res = chunk_list[target_order];
	chunk_list[target_order] = chunk_list[target_order]->pp_link;
	//assert(chunk_list[target_order] == NULL);
	while(target_order > order){
		target_order--;
		struct Page* tmp_p = res + (1 << target_order);
		//cprintf("%d\n", target_order);
		tmp_p-> pp_link = chunk_list[target_order];
		chunk_list[target_order] = tmp_p ;
	}
	//assert(target_order > order);
	if(alloc_flags & ALLOC_ZERO)
			memset(page2kva(res), '\0', PGSIZE * (1 << order));
...
{
	// Fill this function in
	pp->pp_link = chunk_list[order];
	chunk_list[order] = pp;
	struct Page * tmp_p = chunk_list[order]->pp_link;
	struct Page * pre_tmp_p =  chunk_list[order];

	while(order < MAXORDER && tmp_p != NULL){

		if(tmp_p == pp + (1 << order)){
			pre_tmp_p->pp_link = tmp_p->pp_link;
			chunk_list[order] = chunk_list[order]->pp_link;

			tmp_p -> pp_link = NULL;
			pp->pp_link = chunk_list[order + 1];
			chunk_list[order + 1] = pp;

			order++;
			tmp_p = chunk_list[order]->pp_link;
			pre_tmp_p =  chunk_list[order];
		}else if(tmp_p == pp - (1 << order)){
			pre_tmp_p->pp_link = tmp_p->pp_link;
			chunk_list[order] = chunk_list[order]->pp_link;

			pp -> pp_link = NULL;
			tmp_p->pp_link = chunk_list[order + 1];
			chunk_list[order + 1] = tmp_p;
...
{
	// Fill this function in
	pde_t* target_pde = (pde_t*)(pgdir + PDX(va));

	// page table page not exist yet
	if(!(*target_pde & PTE_P)){
		// If this is true, and create == false, then pgdir_walk returns NULL.
		if(!create) return NULL;
		// Otherwise, pgdir_walk allocates a new page table page with page_alloc.
		//ALLOC_ZERO return the PA of the new page (info)
		struct Page* new_ptp = (struct Page*)page_alloc(ALLOC_ZERO);
		//    - If the allocation fails, pgdir_walk returns NULL.
		if(!new_ptp) return NULL;
		//    - Otherwise, the new page's reference count is incremented,
		//clear in page_alloc
		new_ptp->pp_ref ++;
		*target_pde = page2pa(new_ptp) | PTE_P | PTE_W | PTE_U;
	}
	//pde -> pte -> pa -> kva
	pte_t* res = (pte_t*)KADDR(PTE_ADDR(*target_pde)) + PTX(va);
	return res;
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  int cursor = 0;
	while(cursor < size){
		pte_t* tmp_pte = pgdir_walk(pgdir, (const void *)(va + cursor), 1);
		*tmp_pte = (pa + cursor) | perm | PTE_P;
		cursor += PGSIZE;
	}
}
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while(cursor < size){
		//pte_t* tmp_pte = pgdir_walk(pgdir, (const void *)(va + cursor), 1);
		//*tmp_pte = (pa + cursor) | perm | PTE_P | PTE_PS;
		*(pgdir + PDX(va + cursor)) = (pa + cursor) | perm | PTE_P | PTE_PS;
 		cursor += PTSIZE;
	}
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	//   - If necessary, on demand, a page table should be allocated and inserted
	//     into 'pgdir'.
	pte_t* target_pte = pgdir_walk(pgdir, va, 1);
	//   -E_NO_MEM, if page table couldn't be allocated
	if(!target_pte) return -E_NO_MEM;
	//   - pp->pp_ref should be incremented if the insertion succeeds.
	pp->pp_ref ++;
	//   - If there is already a page mapped at 'va', it should be page_remove()d.
	if(*target_pte & PTE_P) page_remove(pgdir, va);
	// The permissions (the low 12 bits) of the page table entry
	// should be set to 'perm|PTE_P'.
	*target_pte = page2pa(pp) | perm | PTE_P;
...
{
	pte_t* target_pte = pgdir_walk(pgdir, va, 0);
	// If pte_store is not zero, then we store in it the address
	if(pte_store)
		*pte_store = target_pte;
	if(target_pte && (*target_pte & PTE_P))
		//pte -> pa -> page*
		return pa2page(PTE_ADDR(*target_pte));
	// Fill this function in
	return NULL;
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	struct Page* target_page = page_lookup(pgdir, va, &target_pte);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t index = ROUNDDOWN(start, PGSIZE);
	perm |= PTE_P;
	for(;index < end; index += PGSIZE){
		...
		pte_t* tmp_pte = pgdir_walk(env->env_pgdir, (void*)index, 0);
		if(tmp_pte == NULL || (*tmp_pte & perm) != perm ){
			if(index >= start)
				user_mem_check_addr = index;
			else
				user_mem_check_addr = start;
