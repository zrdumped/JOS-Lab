...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
	struct  Env* e;
	if(envid2env(envid, &e, 1) < 0 || e == NULL)
		return -E_BAD_ENV;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct Page* p = page_lookup(srce->env_pgdir, srcva, &pte);
	if(p == NULL) {
		//panic("1");
		return -E_INVAL;
	}
...
static int
sys_env_exec_new_env(envid_t envid){
	struct Env *from;
	if(envid2env(envid, &from, 0) < 0)
		return -E_BAD_ENV;

	struct Trapframe tmp_tf = curenv->env_tf;
	curenv->env_tf = from->env_tf;
	from->env_tf = tmp_tf;

	pte_t* tmp_pgdir = curenv->env_pgdir;
	curenv->env_pgdir = from->env_pgdir;
	from->env_pgdir = tmp_pgdir;
...
sys_sbrk(uint32_t inc)
{
	...
	region_alloc(curenv, (void*)(curenv->crt_break - inc), inc);
	curenv->crt_break = (uintptr_t)ROUNDDOWN((curenv->crt_break - inc), PGSIZE);
	return curenv->crt_break;
}
...
//call the e1000 to transmit data
static int 
sys_e1000_transmit(char* data, int length)
{
	user_mem_check(curenv, data, length, PTE_U | PTE_P);
	return e1000_transmit(data, length);
}

static int
sys_e1000_receive(char* data)
{
	user_mem_check(curenv, ROUNDDOWN(data, PGSIZE), PGSIZE, PTE_U | PTE_P | PTE_W);
	return e1000_receive(data);
}

static int
sys_e1000_mac(char* mac)
{
	user_mem_check(curenv, mac, 6, PTE_W);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch(syscallno){
		...
		case SYS_env_set_trapframe:
			...
			break;
		case SYS_env_exec_new_env:
			ret = sys_env_exec_new_env((envid_t)a1);
			break;
		case SYS_time_msec:
			ret = sys_time_msec();
			break;
		case SYS_e1000_transmit:
			ret = sys_e1000_transmit((char*)a1, (int)a2);
			break;
		case SYS_e1000_receive:
			ret = sys_e1000_receive((char*)a1);
			break;
		case SYS_e1000_mac:
			ret = sys_e1000_mac((char*)a1);
			break;
		default:
			ret = -E_INVAL;
			break;
